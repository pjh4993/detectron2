...
class COCOeval:
    ...
    def evaluate(self):
        ...
        self.pious = {(imgId, catId): computeIoU(imgId, catId, prop=True) \
                        for imgId in p.imgIds
                        for catId in catIds}
    ...
    def computeIoU(self, imgId, catId, prop=False):
        p = self.params
        if p.useCats:
        ...
        if p.iouType == 'segm':
            ...
            if prop:
                d = [d['proposal_boxes'] for d in dt]
            else:
    ...
    def computeGTCenterness(self, imgId, catId, prop=False):
        p = self.params
        if p.useCats:
            gt = self._gts[imgId,catId]
            dt = self._dts[imgId,catId]
        ...
        if p.iouType == 'segm':
            ...
            g = [g['bbox'] for g in gt]
            da = [d['anchor'] for d in dt]
            dc = [d['centerness'] for d in dt]
        else:
            raise Exception('unknown iouType for iou computation')
        
        if len(g) == 0 or len(da) == 0 :
            return np.array([])
        g = np.array(g)
        g[:,2:4] += g[:,0:2]
        g[:,0:2] *= -1
        ...
        #g = np.expand_dims(g,axis=1).repeat(da.shape[0],axis=1)
        da = np.expand_dims(da, axis=1).repeat(g.shape[0],axis=1)
        ltrb = g + da
    ...
    def evaluateImg(self, imgId, catId, aRng=None, boxRng=None, levelRng=None, cErrRng=None, maxDet=None, prop=False):
        '''
        ...
        if prop is False:
            ...
            ious = self.pious[imgId, catId][:, gtind] if len(self.pious[imgId, catId]) > 0 else self.pious[imgId, catId]
        
        cErr = self.cErr[imgId, catId][:, gtind] if len(self.cErr[imgId, catId]) > 0 else self.cErr[imgId, catId]
        """
        b = np.array([d['box_area']<boxRng[0] or d['box_area']>boxRng[1] for d in dt])
        l = np.array([d['level']<levelRng[0] or d['level']>levelRng[1] for d in dt])
        b = np.logical_or(b, l)
        if len(b): 
            dt = np.array(dt)[~b].tolist()
        else:
            dt = []
        """
                ...
                for dind, d in enumerate(dt) :
                    ...
                    for gind, g in enumerate(gt):
                        ...
                        if ious[dind,gind] < iou:
                            continue
                        if cErr[dind, gind] < cErrRng[0] or cErr[dind, gind] > cErrRng[1]:
                            continue
    ...
    def accumulate(self, p = None):
        ...
        recall      = -np.ones((T,K,A,B,L,M))
        scores      = -np.ones((T,R,K,A,B,L,M))
        box_size      = -np.ones((T,R,K,A,B,L,M))
        """
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[14] = _summarize(1, areaRng='small', boxRng='all', maxDets=self.params.maxDets[2])
            stats[15] = _summarize(1, areaRng='small', boxRng='small', maxDets=self.params.maxDets[2])
            stats[16] = _summarize(1, areaRng='small', boxRng='medium', maxDets=self.params.maxDets[2])
            stats[17] = _summarize(1, areaRng='small', boxRng='large', maxDets=self.params.maxDets[2])
            stats[18] = _summarize(1, areaRng='medium', boxRng='all', maxDets=self.params.maxDets[2])
            stats[19] = _summarize(1, areaRng='medium', boxRng='small', maxDets=self.params.maxDets[2])
            stats[20] = _summarize(1, areaRng='medium', boxRng='medium', maxDets=self.params.maxDets[2])
            stats[21] = _summarize(1, areaRng='medium', boxRng='large', maxDets=self.params.maxDets[2])
            stats[22] = _summarize(1, areaRng='large', boxRng='all', maxDets=self.params.maxDets[2])
