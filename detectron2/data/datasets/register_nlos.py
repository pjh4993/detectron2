...
def register_nlos_panoptic_separated(
    name, metadata, image_root, panoptic_root, panoptic_json, sem_seg_root, instances_json
):
    """
    Register a NLOS panoptic segmentation dataset named `name`.
    The annotations in this registered dataset will contain both instance annotations and
    semantic annotations, each with its own contiguous ids. Hence it's called "separated".

    It follows the setting used by the PanopticFPN paper:

    1. The instance annotations directly come from polygons in the NLOS
       instances annotation task, rather than from the masks in the NLOS panoptic annotations.
    ...
    DatasetCatalog.register(
        ...
        lambda: merge_to_panoptic(
            load_nlos_json(instances_json, image_root, panoptic_name),
    ...
    MetadataCatalog.get(panoptic_name).set(
        ...
        evaluator_type="nlos_panoptic_seg",
        **metadata
    )
