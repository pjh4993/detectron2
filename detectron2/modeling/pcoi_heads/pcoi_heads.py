...
PCOI_HEADS_REGISTRY = Registry("PCOI_HEADS")
PCOI_HEADS_REGISTRY.__doc__ = """
Registry for PCOI heads in a generalized P-CNN model.
PCOOIHeads take feature maps and part proposal clusters, and
perform per-cluster computation.

The registered object will be called with `obj(cfg, input_shape)`.
The call is expected to return an :class:`PCOIHeads`.
"""

logger = logging.getLogger(__name__)

def build_pcoi_heads(cfg, input_shape):
    """
    Build PCOIHeads defined by `cfg.MODEL.PCOI_HEADS.NAME`.
    """
    name = cfg.MODEL.PCOI_HEADS.NAME
    return PCOI_HEADS_REGISTRY.get(name)(cfg, input_shape)
