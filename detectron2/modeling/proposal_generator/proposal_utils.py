...
def find_top_ppn_proposals(
    ...
    """
    For each feature map, select the `pre_nms_topk` highest scoring proposals,
    clip proposals, and remove small boxes. Return the `post_nms_topk`
    highest scoring proposals among all the feature maps for each image.
    ...
    for _, (proposals_i, logits_i, anchors_i, locales_i) in enumerate(zip(proposals, pred_objectness_logits, anchors, locales)):
        Hi_Wi_A = logits_i.shape[1]
        num_proposals_i = min(pre_nms_topk, Hi_Wi_A)
        ...
        topk_proposals_i = proposals_i[batch_idx[:, None], topk_idx]  # N x topk x 4
        topk_anchors_i = anchors_i.tensor[topk_idx]  # N x topk x 4
        topk_locales_i = locales_i.tensor[topk_idx]

        topk_proposals.append(topk_proposals_i)
        topk_scores.append(topk_scores_i)
        topk_anchors.append(topk_anchors_i)
        topk_locales.append(topk_locales_i)

    # 2. Concat all levels together
    topk_scores = cat(topk_scores, dim=1)
    topk_proposals = cat(topk_proposals, dim=1)
    topk_anchors = cat(topk_anchors, dim=1)
    topk_locales = cat(topk_locales, dim=1)
    ...
    for n, image_size in enumerate(image_sizes):
        ...
        scores_per_img = topk_scores[n]
        anchors_per_img = Boxes(topk_anchors[n])
        locales_per_img = topk_locales[n]
        ...
        if not valid_mask.all():
            ...
            boxes = boxes[valid_mask]
            locales_per_img = locales_per_img[valid_mask]
        boxes.clip(image_size)

        # filter empty boxes
        keep = boxes.nonempty(threshold=min_box_size)
        if keep.sum().item() != len(boxes):
            locales_per_img =  locales_per_img[keep]

        res = Instances(image_size)
        res.locales_per_img = locales_per_img[keep]
        results.append(res)
    return results
